...
# ==============================================================================
try:
    from .node_mappings import NODE_TRANSLATION_TABLE
except ImportError:
    print("[GeoNeural] Warning: node_mappings.py not found. Using empty dict.")
...
def serialize_single_tree(node_tree, nodes_to_process=None, is_compact=False):
    ...
    for node in nodes_to_process:
        ...
        node_data["inputs"] = node_inputs_data
                ...
                if target_sock:
                    try: 
                        if hasattr(target_sock, "type") and target_sock.type == 'OBJECT' and isinstance(v, str):
                            target_sock.default_value = bpy.data.objects.get(v)
                        elif hasattr(target_sock, "type") and target_sock.type == 'COLLECTION' and isinstance(v, str):
                            target_sock.default_value = bpy.data.collections.get(v)
                        elif hasattr(target_sock, "type") and target_sock.type == 'MATERIAL' and isinstance(v, str):
                            target_sock.default_value = bpy.data.materials.get(v)
                        elif hasattr(target_sock, "type") and target_sock.type == 'IMAGE' and isinstance(v, str):
                            target_sock.default_value = bpy.data.images.get(v)
                        else:
                            target_sock.default_value = v
                    except: pass
